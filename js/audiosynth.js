...
// from databse/scale
// in the current key
// (Requires a "#hearKeyButton")
//***********************
window.addEventListener("load", loadAudio);
var playing = false;

// Get notes for the current song and key from the server
function playMelody() {
    if (playing) {
        return;
    }
    ...
    let songEntity = {
        ...
        "songName": melodySongData.songName
    }

    if (melodySongData.hasChords) {
       songEntity.key = melodySongData.key
    }
    ...
    fetch(dbUrl + "/melody",
        ...
        if (noDBConnection(responseJson)) {
            get("hearKeyButton").classList.remove("playing");
            playing = false;
            return;
        }

        playNotes(responseJson.notes);
    });
}

// Play notes from the server response
function playNotes(notes) {
    ...
    setTimeout(function () { playNotes(notes) }, note.length * 1000);

    if (!note.isRest) {
        // Hold short notes for a little longer than their time
        let playLength = Math.sqrt(.3 + Math.pow(note.length, 2)) * 2;

        Synth.play("piano", note.letter, note.octave, playLength);
    }
}

// Setup all html that has to do with audio
function loadAudio() {
    Synth.setVolume(.6);
    get("hearKeyButton").onclick = playMelody;
}
